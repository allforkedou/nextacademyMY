...
class BoggleBoard
  ...
  def include?(word)
    word = word.upcase.gsub('QU','Q') #to handle the case when the word contains QU
    create_hash(word)
    ...
    if include_helper(word, check_hash(word[0]))
      print 'The final path: ' + print_visited_path
      puts
      return true
    else
      return false
  ...
  def print_visited_path
    @visited.each do |vector|
      return vector.to_s.delete('Vector')
    end
  end
  ...
  puts board.include?('QU')
  # puts board.include?('ACE')
