...
module Database
  ...
  class Model
    ...
    def self.where(query, *args)
      Database::Model.execute("SELECT * FROM #{(self.class).downcase}s WHERE #{query}", *args).map do |row|
        self.new(row)
      end
    end
    ...
    def insert!
      ...
      insert_sql = "INSERT INTO #{(self.class).downcase}s (#{fields.join(',')}) VALUES (#{marks})"
    ...
    def update!
      ...
      update_sql = "UPDATE #{(self.class).downcase}s SET #{update_clause} WHERE id = ?"
